...
def pdf_to_images_b64(pdf_bytes: bytes, max_pages: int = MAX_PAGES) -> list[str]:
    ...
    for page_num in range(pages_to_process):
        page = doc[page_num]
        # 150 DPI — מספיק לקריאה ברורה, לא כבד מדי
...
3. כלול את כל השורות ללא יוצא מן הכלל, כולל שורת הסיכום האחרונה בטבלא ב
4. שם הקרן — קח מכותרת הדוח הראשית (לדוגמה: "אלטשולר שחם פנסיה מקיפה")
5. אם שדה לא קיים, החזר null
