...
def process_audit_v29(client, text):
    ...
    TABLE D RULES (מסלולי השקעה):
    - A track name ('מסלול') may span TWO lines in the source text.
      In that case, JOIN the two lines with a space into a SINGLE string for the 'מסלול' field.
    - The 'תשואה' value ALWAYS belongs to the track whose name appears on the line(s) immediately above it.
    - Never leave 'תשואה' empty if a number appears next to or below the track name.
    - Example: if the text reads "מניות\nחו\"ל\n12.5%" then מסלול = "מניות חו\"ל", תשואה = "12.5%".

    TABLE E RULES (פירוט הפקדות):
    - For every REGULAR (non-summary) row:
        * 'מועד' = the full deposit date INCLUDING the day, e.g. "05/03/2024". Copy exactly as it appears.
        * 'חודש' = the salary month WITHOUT a day, e.g. "03/2024". Copy exactly as it appears.
        * Do NOT leave these fields empty for regular rows.
    - For the SUMMARY row only (סה"כ):
    ...
    res = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are a mechanical OCR tool. You copy characters exactly. You do not use logic, you do not round, and you do not flip numbers."},
